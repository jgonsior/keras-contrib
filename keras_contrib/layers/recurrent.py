...
class RNN(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.state_spec is not None:
            ...
            self.state_spec = [
                InputSpec(shape=(None, dim)) for dim in state_size
            ]
        if self.stateful:
            self.reset_states()
        self.built = True
    ...
    def get_initial_state(self, inputs):
        ...
        if hasattr(self.cell.state_size, '__len__'):
            return [
                K.tile(initial_state, [1, dim]) for dim in self.cell.state_size
            ]
        else:
                ...
                if value.shape != (batch_size, dim):
                    raise ValueError(
                        ...
                        + self.name + ': expected shape=' + str(
                            (batch_size,
                             dim)) + ', found shape=' + str(value.shape))
    ...
    def call(self, inputs, states, training=None):
        ...
        if 0 < self.dropout < 1 and self._dropout_mask is None:
            self._dropout_mask = _generate_dropout_mask(
                K.ones_like(inputs), self.dropout, training=training)
...
class InsideLSTM(RNN):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        self.cell._dropout_mask = None
        self.cell._recurrent_dropout_mask = None
        return super(InsideLSTM, self).call(
            inputs, mask=mask, training=training, initial_state=initial_state)
